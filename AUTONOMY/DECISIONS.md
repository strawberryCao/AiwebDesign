...
## ADR-009 — Executable queue state machine
**Status:** accepted  
**Decision:** `AUTONOMY/STATE_MACHINE.md` and `AUTONOMY/QUEUE.json` schema v2 form the executable project-control state. Queue items have explicit owners, statuses, dependencies, direct evidence, one next action, acceptance/abandon conditions, attempt history and strategy versions.  
**Reason:** Natural-language prompt rules alone did not prevent repeated generic refusals, stale ready items, unresolved owner handoffs or identical retries. A persisted state machine makes selection, continuation, CI waiting, recovery and escalation observable and reviewable across stateless scheduled runs.  
**Consequence:** Runs must resume active checkpoints before new work, synchronize queue/next-objective/state fields, and change strategy after three materially equivalent failures. The primary controller and guard govern the full queue at the end of each run; prompts remain stable operating kernels instead of accumulating every dynamic task detail.
